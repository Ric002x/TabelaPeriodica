...
class ElementListViewTest(TableElementsBaseTest):
    def setUp(self) -> None:
        ...
        self.url_list = reverse('table_elements:elements_list')
        self.url_search = reverse('table_elements:search_element')
        return super().setUp()
    ...
    def test_list_view_status_code_404_if_no_elements(self):
        response = self.client.get(self.url_list)
    ...
    def test_list_view_search_page_find_element(self):
        self.make_element()
        response = self.client.get(self.url_search + '?q=Hélio')
        self.assertIn('Hélio', response.content.decode('utf-8'))
